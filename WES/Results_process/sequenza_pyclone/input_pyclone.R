...
MLS <- read.table("MLS_9.hg38_multianno.txt",sep = "\t",header = TRUE)


MLS2  <-MLS[,c(7,60:70)]

colnames(MLS2 ) <- c("Gene","CHROM","POS","ID","REF","ALT","QUAL","FILTER","INFO","FORMAT","MLS","NL")
...
somatic <- data.frame(mutation_id=paste(MLS2$CHROM,MLS2$POS,MLS2$Gene, sep=":"), 
              ...
              ref_counts=sapply(
                sapply(sapply(strsplit(MLS2$MLS,":"),
                              function(c){c[2]}),
...
sample_CN = read.table("MLS_9_segments.txt", header = T,sep = "\t",stringsAsFactors = FALSE)
...
write.table(merge, file = "MLS_9.tsv", row.names=FALSE, sep="\t",quote = FALSE)
